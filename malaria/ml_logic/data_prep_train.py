...
path = os.path.join(dir,'malaria_data','training.json')
with open(path) as f:
    data = json.load(f)
...
# create two lists, which will become our X and ys
    ...
    #        make numpy files
    np.save('train_x_values.npy', np.array(spliced_cells))
    np.save('train_y_values.npy', np.array(spliced_cells_category))
...
return_resized_cell_array(square_enough_good_size_drop_difficult, n_cells=1000)
