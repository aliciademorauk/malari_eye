...
def load_json(filename):
    ...
    print(base_filepath_to_json_images)
...
def return_resized_cell_array(iter_df, im_col = True, n_cells=1000) -> (np.ndarray, np.ndarray):
...
# create two lists, which will become our X and ys
    ...
    #        create f string from path column to open each large image in np.array using Pillow image package
        img = Image.open(f"{base_filepath_to_json_images}{iter_df['path'][index]}")
...
def save_array(source_filename, im_col, ncells = 1000):
    if im_col:
        appx = "_COL"
    else:
        appx = "_B&W"
    ...
    np.save(filename_path+"_X_"+str(ncells),X)
    np.save(filename_path+"_y_"+str(ncells),y)
...
save_array("test.json", im_col = True)
